...
(function () {
  ...
  const mobileItems = nav.items.filter(item => !item.desktopOnly).map(mobileLink);
  ...
  // The module's logo: an image when the config gives one (the portal's BRANDING.logoImage), else the emoji
  const logoIcon = nav.logo.image
    ? `<img class="logo__image" src="${url(nav.logo.image)}" alt="">`
    : `<span class="logo__icon">${nav.logo.icon}</span>`;
  ...
  const html = `
    ...
    <header class="header">
      <div class="logo">
        ${logoIcon}
        <span class="logo__text">${nav.logo.text}</span>
      </div>
    ...
    <nav class="mobile-nav" id="mobile-nav">
      <div class="mobile-nav__header">
        <div class="mobile-nav__logo">${nav.logo.image ? `<img class="logo__image" src="${url(nav.logo.image)}" alt="">` : `<span>${nav.logo.icon}</span>`}<span>${nav.logo.text}</span></div>
