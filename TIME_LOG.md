...
The 2026-09-22 row covers: renaming "Inspection Randonnée" to "Inspections", moving the Montée/Descente
toggle from a dashboard-local control to the shared header switcher (`kind.js`, all 5 inspection pages),
clarifying the "Gestion des données" tab's scope in its own text, and the admin Statistiques season
navigator (prev/next, 12-month windows) - fully continuous, no gaps over 5 minutes the whole stretch.
